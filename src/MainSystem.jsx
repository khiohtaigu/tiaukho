...
export default function MainSystem() {
  ...
  const handleTouchSwapSelection = async (dayIdx, periodId, item) => {
    ...
    if (!draggedItem) {
        ...
        if (isInvincibleMode) {
            ...
            const idx = nextSchedules.findIndex(s => s.id === draggedItem.id);
            if (idx !== -1) {
                nextSchedules[idx] = { ...nextSchedules[idx], day: dayIdx, period: periodId };
  ...
  const handleFileUpload = (e) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        const newSchedules = []; const newTeachers = []; const newConstraints = []; const newDomainWarnings = []; 
        const teacherCache = {}; const classSet = new Set(); const scheduleCheck = new Set(); const nativeTeachersNames = new Set();
        ...
        workbook.SheetNames.forEach(sheetName => {
          ...
          if (sheetName === "領域時間") {
            json.forEach(row => {
                const domain = String(row['領域'] || '').trim(), d = dayMap[String(row['星期']).trim()], p = parseInt(String(row['節次']).replace(/[^0-9]/g, ''));
                if (domain && d !== undefined && p) newDomainWarnings.push({ domain, day: d, period: periodMap[p] || p, desc: (domain === "導師" || domain === "團體活動" || domain === "輔導課") ? domain : "領域時間" });
            });
          } else if (sheetName === "禁區設定") {
            json.forEach((row, idx) => {
              const typeRaw = String(row['類型'] || ''); if (!typeRaw) return;
              let type = 'classes', target = typeRaw;
              if (typeRaw.includes('全校')) type = 'all';
              else if (typeRaw.includes('高一')) { type = 'grade'; target = '1'; }
              else if (typeRaw.includes('高二')) { type = 'grade'; target = '2'; }
              else if (typeRaw.includes('高三')) { type = 'grade'; target = '3'; }
              newConstraints.push({ id: `C${idx}-${Date.now()}`, type, target: String(target), days: [dayMap[String(row['星期']).trim()] ?? 0], periods: [periodMap[parseInt(String(row['節次']).replace(/[^0-9]/g, ''))] ?? 1], desc: String(row['說明'] || '') });
            });
          } else {
            json.forEach(row => {
              const tName = String(row['老師姓名'] || '').trim(); if (!tName || tName === 'nan') return;
              if (!teacherCache[tName]) {
                const adminRole = String(row['行政職稱'] || row['行政職務'] || '').trim();
                const adjunctInfo = String(row['兼課/代理'] || '').trim();
                teacherCache[tName] = { 
                  id: `T${newTeachers.length+1}`, name: tName, order: parseInt(row['編號'])||999, 
                  domain: String(row['領域']||''), subject: String(row['學科']||''), 
                  adminRole: (adminRole === 'nan' || adminRole === '正式') ? '' : adminRole, 
                  isAdjunct: adminRole.includes('兼課') || adjunctInfo.includes('兼課'),
                  isHomeroom: row['是否導師']==='是', teachesNative: nativeTeachersNames.has(tName) 
                };
              ...
              if (d!==undefined && p!==undefined) {
                ...
                if (!scheduleCheck.has(key)) {
                  ...
                  newSchedules.push({ id: `S${newSchedules.length}`, teacherName: tName, classId: cId, subject: String(row['課表原始名稱'] || row['表原始名稱'] || '課程'), day: d, period: p });
  ...
  const handleFileExport = () => {
    try {
      ...
      const exportData = schedules.map(s => {
        ...
        return {
          '編號': t?.order || '', '領域': t?.domain || '', '學科': t?.subject || '', '老師姓名': s.teacherName,
          '兼課/代理': t?.isAdjunct ? '兼課' : '正式', '行政職稱': t?.adminRole || '', '是否導師': t?.isHomeroom ? '是' : '否',
          '課表原始名稱': s.subject, '班級': s.classId, '星期': DAYS[s.day], '節次': revMap[s.period] || s.period
        };
      });
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(exportData), "課表資料");
      XLSX.writeFile(wb, `${currentSchool?.name || '學校'}_完整備份_${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const PreviewGrid = ({ teacherName, moves }) => (
      <div className="bg-white rounded-2xl border-2 border-slate-400 overflow-hidden shadow-sm font-sans flex-1 text-slate-900 font-black">
        <div className="bg-[#475569] p-2 font-black text-center text-white border-b-2 border-slate-400 flex items-center justify-center gap-2"><User size={16} className="text-white"/> <span className="text-base font-serif font-black text-white">{teacherName} 老師</span></div>
  ...
  if (landingStage === 0) {
    return (
      <div className="flex flex-col h-screen w-full bg-[#fdfaf1] items-center justify-center overflow-hidden font-serif text-slate-900">
        <div className="bg-white rounded-[2rem] md:rounded-[4rem] shadow-2xl border-[4px] md:border-[6px] border-[#fbda8b] p-8 md:p-20 text-center animate-in zoom-in duration-700 max-w-[95%] md:max-w-4xl w-full">
          ...
          <button onClick={() => setLandingStage(1)} className="bg-[#fbda8b] hover:bg-[#f9cf6a] text-[#1e3a8a] px-8 py-4 md:px-14 md:py-7 rounded-[1.5rem] md:rounded-[2.5rem] text-2xl md:text-4xl font-black shadow-xl transition-all flex items-center gap-3 md:gap-5 mx-auto active:scale-95 font-sans">點擊進入 <ArrowRight size={32} /></button>
        </div>
        <div className="mt-8 md:mt-12 flex flex-col items-center gap-3 px-4 text-center">
            <div className="flex items-center gap-2 text-slate-500 font-black text-sm md:text-lg bg-white/80 px-4 md:px-6 py-2 rounded-full shadow-sm border border-slate-200 font-black">
            ...
            <div className="text-slate-400 font-bold text-xs md:text-base tracking-widest uppercase font-sans">© {currentYear} 天才小調手 X 耀毅. All Rights Reserved.</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col h-screen w-full bg-[#f1f5f9] items-center justify-center overflow-hidden font-sans text-slate-900 px-4 font-black">
        <div className="bg-white rounded-[2.5rem] md:rounded-[3.5rem] shadow-2xl border-2 border-slate-200 p-8 md:p-16 text-center animate-in slide-in-from-bottom-8 duration-700 max-w-lg w-full relative overflow-hidden font-black">
          ...
          <h3 className="text-3xl md:text-5xl font-black text-slate-800 mb-8 md:mb-12 tracking-tight font-serif">鳳山高級中學</h3>
          ...
          <button onClick={() => setLandingStage(0)} className="mt-8 md:mt-10 text-slate-400 font-bold text-base md:text-lg hover:text-slate-600 transition-colors flex items-center justify-center gap-2 mx-auto">← 返回首頁</button>
        </div>
        {showLoginModal && (
            <div className="fixed inset-0 bg-slate-900/90 backdrop-blur-md flex items-center justify-center z-50 p-4">
                <div className="bg-white rounded-[2.5rem] md:rounded-[3rem] p-8 md:p-12 max-w-md w-full shadow-2xl border-4 border-[#1e40af] relative font-black font-black">
                    <button onClick={() => setShowLoginModal(false)} className="absolute top-6 right-6 md:top-8 md:right-8 text-slate-300 hover:text-slate-600"><X size={28}/></button>
                    ...
                    <h3 className="text-2xl md:text-3xl font-black text-center text-slate-800 mb-4 font-serif font-black">管理員驗證</h3>
                    ...
                    <button onClick={handleAdminLogin} className="w-full bg-[#1e40af] text-white py-4 md:py-5 rounded-xl md:rounded-2xl font-black text-lg md:text-xl hover:bg-blue-900 font-serif">確認登入</button>
                </div>
            </div>
        )}
      </div>
  ...
  return (
    <div className="flex h-screen w-full bg-[#f1f5f9] text-slate-900 overflow-hidden font-serif font-black relative">
      ...
      <aside className={`w-80 bg-white border-r border-slate-300 flex flex-col shadow-xl z-40 shrink-0 font-sans text-slate-900 absolute md:relative h-full transition-transform duration-300 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="p-6 bg-[#1e3a8a] text-white shrink-0 font-black font-black">
        ...
        <div className="p-4 border-b border-slate-300 bg-white shrink-0 font-black font-black"><div className="relative"><Search className="absolute left-3 top-3 text-slate-400" size={20} /><input type="text" placeholder="搜尋..." className="w-full pl-11 pr-4 py-3 bg-slate-100 border-none rounded-xl text-base font-bold outline-none placeholder-slate-400 text-slate-900" onChange={(e) => setSearchTerm(e.target.value)} /></div></div>
        <div className="flex-1 overflow-y-auto custom-scrollbar p-3 bg-white font-black font-black">
          {sidebarMode === 'teacher' ? (
            <div className="space-y-2 pb-10 font-black">
              {SIDEBAR_ORDER.map(key => {
                ...
                if (members) {
                  ...
                  return (
                    <div key={key}>
                      <button onClick={() => toggleMain(key)} className="w-full flex items-center justify-between p-4 hover:bg-slate-100 rounded-xl group font-black text-lg text-slate-800 leading-none"><div className="flex items-center gap-3"><BookOpen size={20} className="text-blue-600 font-black"/>{key === '本土語' ? key : key + '科'}</div>{isExpanded ? <ChevronDown size={20}/> : <ChevronRight size={20}/>}</button>
                ...
                if (domains) {
                  const isExpanded = activeMainKey === key || searchTerm !== '';
                  return (
                    <div key={key} className="mb-2 bg-slate-50 rounded-2xl border border-slate-200 overflow-hidden shadow-sm font-black font-black">
                      ...
                      {isExpanded && <div className="p-2 space-y-1 font-black">{Object.entries(domains).map(([sub, subMembers]) => {
                        ...
                        return (<div key={sub} className="font-black"><button onClick={(e) => toggleSub(e, key + sub)} className="w-full flex items-center justify-between px-4 py-3 hover:bg-white rounded-xl text-base font-black text-slate-600 shadow-sm border border-transparent leading-none"><span>{sub}</span>{isSubExp ? <ChevronDown size={18}/> : <ChevronRight size={18}/>}</button>
            ...
            <div className="space-y-4 p-1 pb-10 font-black">
              {Object.entries(groupedClasses).map(([grade, list]) => (
                <div key={grade} className="mb-4 bg-white rounded-2xl border-2 border-slate-200 shadow-sm overflow-hidden text-slate-900 font-black font-black text-slate-900">
                  ...
                  {gradeExpanded[grade] && <div className="p-4 grid grid-cols-3 gap-3 font-black font-black">
        ...
        <div className="p-4 border-t border-slate-200 bg-slate-50 shrink-0 flex flex-col gap-3 text-slate-900 font-sans font-black font-black">
            <div className="flex items-center justify-center gap-2 py-2 bg-white rounded-xl border border-slate-200 shadow-inner font-black">
                <Eye size={16} className="text-blue-500 font-black" />
                <span className="text-xs font-black text-slate-500">累積訪問<span className="hidden md:inline">次數</span>：<span className="text-blue-700 font-sans">{visitCount.toLocaleString()}</span></span>
            </div>
            {isAdmin && (
                <>
                <button onClick={handleFileExport} className="flex items-center gap-3 px-4 py-3 bg-white border-2 border-slate-200 rounded-2xl cursor-pointer hover:border-green-500 hover:bg-green-50 group shadow-sm leading-none"><Download className="text-slate-400 group-hover:scale-110 transition-all font-black" size={24} /><span className="font-black text-slate-600">配課表匯出</span></button>
                <label className="flex items-center gap-3 px-4 py-3 bg-white border-2 border-slate-200 rounded-2xl cursor-pointer hover:border-blue-500 hover:bg-blue-50 group shadow-sm leading-none"><Settings className="text-slate-400 group-hover:rotate-90 transition-all font-black" size={24} /><span className="font-black text-slate-600">配課表匯入</span><input type="file" accept=".xlsx, .xls" className="hidden font-black" onChange={handleFileUpload} /></label>
                </>
            )}
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col h-screen overflow-hidden bg-slate-50 text-slate-900 font-serif font-black">
        <header className="h-16 md:h-24 bg-white border-b-2 border-slate-200 flex items-center justify-between px-4 md:px-10 shadow-sm shrink-0 z-10 font-sans font-black">
          <div className="flex items-center gap-2 md:gap-6 leading-none font-black font-black">
            <button className="md:hidden p-2 text-slate-600 hover:bg-slate-100 rounded-lg font-black font-black" onClick={() => setIsSidebarOpen(true)}><Menu size={28} /></button>
            ...
            <div className="leading-tight font-serif font-black text-slate-900"><h2 className="text-lg md:text-3xl font-black tracking-tight mb-0.5 leading-none">{sidebarMode === 'teacher' ? (selectedTeacher?.name || '請選取') : (selectedClass?.name || '請選取')} <span className="hidden sm:inline font-black">的週課表</span></h2><span className="text-blue-600 font-bold text-xs md:text-base">{sidebarMode === 'teacher' ? (selectedTeacher?.subject ? `${selectedTeacher.subject}科` : '') : (selectedClass?.grade || '')}</span></div>
          </div>
          <div className="flex gap-2 md:gap-3 leading-none font-sans font-black font-black">
            {isAdmin && (
               <button onClick={() => { setIsInvincibleMode(!isInvincibleMode); setIsEditMode(false); setDraggedItem(null); }} className={`px-3 md:px-6 py-1.5 md:py-2.5 rounded-lg md:rounded-xl font-black text-[10px] md:text-sm transition-all border flex items-center gap-1 md:gap-2 leading-none ${isInvincibleMode ? 'bg-red-600 text-white border-red-700 shadow-lg animate-pulse' : 'bg-white text-red-600 border-red-200 hover:bg-red-50'}`}><ShieldAlert size={14} />{isInvincibleMode ? '停止無敵' : '無敵調課'}</button>
            )}
            {(isAdmin || dbData.isSimEnabled) && (
                <button onClick={() => { setIsEditMode(!isEditMode); setIsInvincibleMode(false); setDraggedItem(null); }} disabled={!isAdmin && !dbData.isSimEnabled} className={`px-3 md:px-6 py-1.5 md:py-2.5 rounded-lg md:rounded-xl font-black text-[10px] md:text-sm transition-all border flex items-center gap-1 md:gap-2 leading-none ${isEditMode ? 'bg-purple-600 text-white border-purple-700 shadow-lg animate-pulse' : (!isAdmin && !dbData.isSimEnabled) ? 'bg-slate-100 text-slate-400 border-slate-200 cursor-not-allowed' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}>{isEditMode ? '停止調課' : (isAdmin ? '智慧調課' : '模擬調課')}</button>
            )}
            {isAdmin && sidebarMode === 'class' && (
                <button onClick={() => { setIsManualSwapMode(!isManualSwapMode); setSwapQueue([]); }} className={`px-3 md:px-6 py-1.5 md:py-2.5 rounded-lg md:rounded-xl font-black text-[10px] md:text-sm transition-all border flex items-center gap-1 md:gap-2 leading-none ${isManualSwapMode ? 'bg-green-600 text-white border-green-700 shadow-lg' : 'bg-white text-slate-600 border-slate-200 hover:bg-slate-50'}`}><ListChecks size={14} /><span className="hidden md:inline">班級課表調動</span><span className="md:hidden">調動</span></button>
            )}
            <button onClick={() => setActiveView('schedule')} className={`px-3 md:px-6 py-1.5 md:py-2.5 rounded-lg md:rounded-xl font-black text-[10px] md:text-sm flex items-center gap-1 md:gap-2 leading-none ${activeView === 'schedule' ? 'bg-[#1e40af] text-white shadow-lg font-black' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 font-black'}`}><BookOpen size={14}/><span>檢視<span className="hidden md:inline">課表</span></span></button>
            {isAdmin && <button onClick={() => setActiveView(activeView === 'settings' ? 'schedule' : 'settings')} className={`px-3 md:px-6 py-1.5 md:py-2.5 rounded-lg md:rounded-xl font-black text-[10px] md:text-sm flex items-center gap-1 md:gap-2 leading-none ${activeView === 'settings' ? 'bg-[#1e40af] text-white shadow-lg font-black' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 font-black'}`}><Settings size={14}/><span>排課<span className="hidden md:inline">設定</span></span></button>}
          </div>
        </header>

        {(isEditMode || isManualSwapMode || isInvincibleMode) && (
            <div className={`border-b px-4 md:px-10 py-2 flex items-center gap-2 md:gap-4 font-black animate-in slide-in-from-top-4 duration-300 font-sans text-[10px] md:text-base ${isManualSwapMode ? 'bg-green-50 text-green-800 border-green-200' : isInvincibleMode ? 'bg-red-50 text-red-800 border-red-200' : 'bg-purple-50 text-purple-800 border-purple-200'}`}>
                <Zap size={16} className="animate-pulse" />
                <span className="truncate">
                    {isManualSwapMode ? `手動：已選 ${swapQueue.length} 站` : isInvincibleMode ? '無敵模式：點擊課程再點位置，可強制衝堂調動。' : `智慧模式：點擊課程，再點目標位置即可。`}
                </span>
                <div className="ml-auto flex gap-2 font-black">
                    {isManualSwapMode && <button onClick={executeManualSwap} className="bg-green-600 text-white px-3 py-1 rounded-lg text-[10px] md:text-xs font-black">完成</button>}
                    <button onClick={() => { setIsManualSwapMode(false); setIsEditMode(false); setIsInvincibleMode(false); setSwapQueue([]); setDraggedItem(null); }} className="bg-slate-200 text-slate-600 px-3 py-1 rounded-lg text-[10px] md:text-xs font-black">取消</button>
                </div>
            </div>
        )}

        {activeView === 'settings' ? (
          <div className="flex-1 p-4 md:p-10 overflow-y-auto bg-slate-50 custom-scrollbar pb-32 font-sans font-black font-black">
            <div className="max-w-4xl mx-auto space-y-6 animate-in slide-in-from-bottom-4 duration-500 font-black">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 font-black font-black text-slate-900">
                <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] shadow-lg border-2 border-slate-200 p-6 md:p-8 relative flex flex-col gap-4 md:gap-6 overflow-hidden">
                    <div className="absolute left-0 top-0 bottom-0 w-2 md:w-3 bg-purple-600"></div>
                    <div className="flex items-center justify-between font-black"><div className={`w-12 h-12 md:w-14 md:h-14 rounded-xl md:rounded-2xl flex items-center justify-center ${dbData.isApplyEnabled ? 'bg-purple-100 text-purple-600 font-black' : 'bg-slate-100 text-slate-400 font-black'}`}><Edit2 size={24} /></div><button onClick={() => handleToggleState('isApplyEnabled')} className={`p-1 rounded-full transition-all w-16 md:w-24 relative ${dbData.isApplyEnabled ? 'bg-purple-600 font-black' : 'bg-slate-300 font-black'}`}><div className={`w-7 h-7 md:w-10 md:h-10 bg-white rounded-full shadow-md flex items-center justify-center transform transition-transform ${dbData.isApplyEnabled ? 'translate-x-8 md:translate-x-12 font-black' : 'translate-x-0 font-black'}`}>{dbData.isApplyEnabled ? <ToggleRight className="text-purple-600 font-black" /> : <ToggleLeft className="text-slate-400 font-black" />}</div></button></div>
                    <div><h3 className="text-xl md:text-2xl font-black flex items-center gap-2 font-serif font-black">填報功能<span className="hidden md:inline font-black">管理</span> {dbData.isApplyEnabled ? <span className="text-[10px] bg-green-500 text-white px-2 py-0.5 rounded-full font-black">開</span> : <span className="text-[10px] bg-red-500 text-white px-2 py-0.5 rounded-full font-black">關</span>}</h3><p className="text-slate-400 font-bold text-xs md:text-sm mt-1 leading-relaxed">控制是否能進入「自主調課表」頁面。</p></div>
                </div>
              ...
              <div id="rule-form" className="bg-white rounded-[1.5rem] md:rounded-[2rem] shadow-lg border-2 border-slate-200 p-6 md:p-10 relative font-serif font-black text-slate-900 font-black">
                <h3 className="text-xl md:text-3xl font-black text-slate-800 flex items-center gap-3 mb-6 md:mb-10 font-serif font-black"> {editingRuleId ? <Edit2 size={24} className="text-orange-600"/> : <Plus size={24} className="text-blue-600 font-black"/>} {editingRuleId ? '修改排課規則' : '新增排課規則'}</h3>
                <div className="space-y-6 md:space-y-8 font-sans font-black">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-10 font-black">
                    <div><label className="block text-xs md:text-sm font-black text-slate-400 mb-2 md:mb-4 tracking-widest uppercase font-black">範圍</label><div className="flex gap-2 font-black"><button onClick={() => setNewRule({...newRule, type:'all'})} className={`flex-1 py-3 md:py-4 rounded-xl border-2 font-black text-xs md:text-base leading-none ${newRule.type==='all'?'bg-blue-50 border-blue-600 text-blue-700 font-black':'bg-white border-slate-100 text-slate-400 font-black'}`}>全校</button><button onClick={() => setNewRule({...newRule, type:'grade'})} className={`flex-1 py-3 md:py-4 rounded-xl border-2 font-black text-xs md:text-base leading-none ${newRule.type==='grade'?'bg-blue-50 border-blue-600 text-blue-700 font-black':'bg-white border-slate-100 text-slate-400 font-black'}`}>年級</button><button onClick={() => setNewRule({...newRule, type:'classes'})} className={`flex-1 py-3 md:py-4 rounded-xl border-2 font-black text-xs md:text-base leading-none ${newRule.type==='classes'?'bg-blue-50 border-blue-600 text-blue-700 font-black':'bg-white border-slate-100 text-slate-400 font-black'}`}>班級</button></div><div className="mt-4 font-black">{newRule.type === 'grade' && <select className="w-full p-3 md:p-4 bg-slate-100 rounded-xl border-none font-black text-sm md:text-lg font-black text-slate-900" value={newRule.target} onChange={e=>setNewRule({...newRule, target:e.target.value})}><option value="1">高一年級</option><option value="2">高二年級</option><option value="3">高三年級</option></select>}{newRule.type === 'classes' && <input type="text" placeholder="例如: 201、202-205" className="w-full p-3 md:p-4 bg-slate-100 rounded-xl border-none font-black text-sm md:text-lg font-black text-slate-900" value={newRule.classList} onChange={e=>setNewRule({...newRule, classList:e.target.value})} />}</div></div>
                    <div><label className="block text-xs md:text-sm font-black text-slate-400 mb-2 md:mb-4 tracking-widest uppercase leading-none font-black">規則說明</label><input type="text" placeholder="說明" className="w-full p-3 md:p-4 bg-slate-100 rounded-xl border-none font-black text-sm md:text-lg h-12 md:h-16 font-black text-slate-900" value={newRule.desc} onChange={e=>setNewRule({...newRule, desc:e.target.value})} /></div>
                  </div>
                  <div className="space-y-4 md:space-y-6 font-black"><label className="block text-xs md:text-sm font-black text-slate-400 tracking-widest uppercase flex items-center gap-2 font-black"><Clock size={16}/> 生效時段</label><div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 font-black"><div className="space-y-4 font-black text-slate-900"><span className="text-[10px] md:text-xs font-black text-slate-400 uppercase tracking-tighter font-black">星期</span><div className="flex flex-wrap gap-2 font-black font-black">{DAYS.map((d, i) => { const isSel = newRule.days.includes(i); return (<button key={d} onClick={() => setNewRule({ ...newRule, days: isSel ? newRule.days.filter(x => x !== i) : [...newRule.days, i]})} className={`px-3 md:px-4 py-1.5 md:py-2 rounded-xl font-black border-2 transition-all text-xs md:text-base font-black ${isSel ? 'bg-blue-600 border-blue-700 text-white font-black' : 'bg-white border-slate-200 text-slate-400 font-black'}`}>{d}</button>);})}</div></div><div className="space-y-4 font-black text-slate-900 font-black font-black"><span className="text-[10px] md:text-xs font-black text-slate-400 uppercase tracking-tighter font-black font-black">節次</span><div className="flex flex-wrap gap-2 font-black font-black font-black">{PERIODS.filter(p => !p.isRest).map((p) => { const isSel = newRule.periods.includes(p.id); return (<button key={p.id} onClick={() => setNewRule({ ...newRule, periods: isSel ? newRule.periods.filter(x => x !== p.id) : [...newRule.periods, p.id]})} className={`px-2.5 md:px-3 py-1 md:py-2 rounded-xl font-black border-2 transition-all text-[10px] md:text-sm font-black font-black font-black ${isSel ? 'bg-indigo-600 border-indigo-700 text-white font-black' : 'bg-white border-slate-200 text-slate-400 font-black'}`}>{p.label.replace(/[^0-9]/g, '')}</button>);})}</div></div></div></div>
                  <div className="flex gap-4 font-black font-black font-black"><button onClick={async () => { if(!newRule.desc || newRule.days.length===0 || newRule.periods.length === 0) return alert('資訊不全'); const targetVal = newRule.type==='grade'?newRule.target:(newRule.type==='classes'?newRule.classList:'全校'); let updated; if (editingRuleId) { updated = constraints.map(c => c.id === editingRuleId ? { ...newRule, id: editingRuleId, target: targetVal } : c); } else { updated = [...constraints, { ...newRule, id: Date.now(), target: targetVal }]; } setConstraints(updated); await setDoc(doc(db, "schools", currentSchool.id), { ...dbData, constraints: updated }); setNewRule({type: 'classes', target: '1', classList: '', days: [], periods: [], desc: ''}); setEditingRuleId(null); }} className={`flex-1 py-4 md:py-6 ${editingRuleId ? 'bg-orange-600 font-black' : 'bg-[#1e3a8a] font-black'} text-white rounded-2xl md:rounded-[2rem] font-black text-lg md:text-2xl shadow-xl transition-all font-serif font-black text-white`}>儲存<span className="hidden md:inline font-black text-white font-black">規則</span></button>{editingRuleId && <button onClick={() => { setEditingRuleId(null); setNewRule({type: 'classes', target: '1', classList: '', days: [], periods: [], desc: ''}); }} className="px-6 md:px-10 py-4 md:py-6 bg-slate-200 text-slate-600 rounded-2xl md:rounded-[2rem] font-black text-lg md:text-2xl shadow-lg font-serif font-black font-black font-black">取消</button>}</div>
                </div>
              </div>

              <div className="bg-white rounded-[1.5rem] md:rounded-[2rem] shadow-lg border-2 border-slate-200 p-6 md:p-10 font-serif font-black text-slate-900 font-black">
                <h3 className="text-xl md:text-3xl font-black text-slate-800 mb-6 md:mb-8 flex items-center gap-3 font-serif font-black font-black font-black"><ListChecks size={28} className="text-blue-600 font-black font-black"/> 目前規則</h3>
                    ...
                    {constraints.map(rule => (
                      <div key={rule.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 md:p-6 bg-slate-50 rounded-2xl border-2 border-slate-100 group gap-4 font-black font-black">
                        <div className="flex gap-4 md:gap-6 items-center font-black">
                          <div className={`px-3 md:px-4 py-1 md:py-2 rounded-xl font-black text-white text-[10px] md:text-base font-black ${rule.type==='all'?'bg-red-500':rule.type==='grade'?'bg-blue-500 font-black':'bg-purple-500'}`}>{rule.type==='all'?'全校':(rule.type==='grade'?`高${rule.target}`:`班級:${rule.target}`)}</div>
                          <div className="space-y-1 font-black"><div className="text-base md:text-xl font-black text-slate-800 font-black font-black">{rule.desc}</div><div className="text-[10px] md:text-sm font-bold text-slate-400 flex items-center gap-2 font-sans font-black"><Clock size={12}/>{rule.days.map(d=>DAYS[d]).join(', ')} • {rule.periods.map(p=>`第${PERIODS.find(per=>per.id===p)?.label.replace(/[^0-9]/g, '')}節`).join(', ')}</div></div>
                        </div>
                        <div className="flex gap-2 sm:opacity-0 group-hover:opacity-100 transition-opacity font-black">
                          <button onClick={() => { setEditingRuleId(rule.id); setNewRule({ ...rule, classList: rule.type==='classes'?rule.target:'' }); document.getElementById('rule-form').scrollIntoView({ behavior: 'smooth' }); }} className="p-2 md:p-3 bg-white border-2 border-slate-200 rounded-xl text-slate-500 hover:text-orange-600 hover:border-orange-500 transition-all shadow-sm font-black font-black"><Edit2 size={16}/></button>
                          <button onClick={async () => { if(window.confirm('確定要刪除？')){ const updated = constraints.filter(c=>c.id!==rule.id); setConstraints(updated); await setDoc(doc(db, "schools", currentSchool.id), { ...dbData, constraints: updated }); } }} className="p-2 md:p-3 bg-white border-2 border-slate-200 rounded-xl text-slate-500 hover:text-red-600 hover:border-red-500 transition-all shadow-sm font-black font-black font-black"><Trash2 size={16}/></button>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          <div className="flex-1 p-2 md:p-8 flex flex-col min-h-0 overflow-hidden bg-slate-50 text-slate-900 font-serif font-black">
            {isLoading ? <div className="flex-1 flex items-center justify-center text-2xl font-black text-slate-300 animate-pulse font-sans font-black font-black font-black font-black">連線中...</div> : 
            <div className="flex-1 bg-white rounded-[1rem] md:rounded-[2.5rem] shadow-2xl border-2 border-slate-300 overflow-hidden flex flex-col font-black font-black">
              <div className="grid grid-cols-6 bg-[#1e293b] text-white shrink-0 border-b-2 border-slate-600 leading-none font-sans font-black font-black">
                <div className="p-2 md:p-4 text-center text-[8px] md:text-[11px] font-black border-r border-slate-600 uppercase tracking-widest leading-relaxed font-black font-black font-black">節次</div>
                {DAYS.map(day => <div key={day} className="p-2 md:p-4 text-center font-black text-[10px] md:text-xl border-r border-slate-600 last:border-r-0 leading-relaxed tracking-wider font-black font-black font-black text-white">{day}</div>)}
              </div>
              <div className="flex-1 flex flex-col min-h-0 overflow-y-auto font-black font-black">
                {PERIODS.map(period => (
                  <div key={period.id} className={`grid grid-cols-6 border-b-2 border-slate-300 last:border-b-0 flex-none min-h-[70px] md:min-h-0 md:flex-1 font-black ${period.isRest ? 'bg-slate-100 h-10 md:h-14 font-black font-black' : ''}`}>
                    ...
                    {period.isRest ? <div className="col-span-5 flex items-center justify-center text-slate-400 text-[10px] md:text-sm font-black tracking-[1em] md:tracking-[3em] uppercase bg-slate-100/50 italic leading-none font-sans font-black font-black">午 休 時 間</div> : 
                      DAYS.map((_, dIdx) => {
                        ...
                        return (
                          <div key={dIdx} 
                            ...
                            className={`border-r border-slate-200 md:border-r-2 md:border-slate-300 last:border-r-0 flex flex-col items-center justify-center text-center transition-all relative overflow-hidden ${lockRule ? 'bg-slate-50 cursor-not-allowed font-black font-black' : 'bg-white font-black'} ${(isEditMode || isManualSwapMode || isInvincibleMode) && !lockRule ? 'cursor-pointer hover:bg-blue-50/30 font-black' : ''} ${swapIdx !== -1 ? 'ring-2 md:ring-4 ring-green-500 z-10 bg-green-50/50 font-black' : ''} ${isTouchSelected ? 'ring-2 md:ring-4 ring-purple-500 z-10 animate-pulse bg-purple-50' : ''} ${isConflict ? 'ring-4 ring-red-500 animate-pulse z-[5] bg-red-50/50' : ''}`}>
                            {swapIdx !== -1 && <div className="absolute top-0.5 left-0.5 w-5 h-5 md:w-7 md:h-7 bg-green-600 text-white rounded-full flex items-center justify-center font-black text-[10px] md:text-xs shadow-lg animate-in zoom-in duration-200 font-black font-black font-black">{swapIdx + 1}</div>}
                            ...
                            {lockRule ? <div className="flex flex-col items-center gap-0.5 md:gap-1 opacity-70 px-0.5 text-slate-500 font-black font-black font-black"><Lock size={12}/><span className="text-[8px] md:text-[14px] font-black uppercase tracking-tighter truncate max-w-full font-sans text-center font-black">{lockRule.desc}</span></div> : 
                              items.map((item, idx) => (
                                <div key={idx} onDoubleClick={() => handleCellDoubleClick(item)} draggable={isEditMode} onDragStart={() => setDraggedItem(item)} className={`w-full px-0.5 flex flex-col items-center justify-center select-none font-black font-black ${isEditMode ? 'cursor-grab active:cursor-grabbing hover:scale-105 transition-transform' : ''}`}>
                                  {sidebarMode === 'teacher' ? (<><div className="font-black text-blue-900 text-2xl md:text-3xl tracking-tighter mb-0.5 md:mb-1 font-sans font-black font-black font-black">{item.classId!=="未知"?item.classId:""}</div><div className="px-1 md:px-3 py-0.5 md:py-1 bg-blue-900 text-white text-[9px] md:text-[11px] font-black rounded md:rounded-lg shadow-sm font-sans uppercase truncate max-w-full font-black text-white font-black">{item.subject}</div></>) : (<div className="w-full flex flex-col items-center px-0.5 font-sans font-black text-center font-black font-black font-black font-black"><div className="font-black text-slate-800 text-[13px] md:text-2xl tracking-tighter leading-tight mb-0.5 truncate max-w-full font-black font-black">{item.subject}</div><div className="px-1 md:px-2 py-0 md:py-0.5 bg-slate-800 text-white text-[11px] md:text-[10px] font-black rounded shadow-sm opacity-90 truncate max-w-[95%] font-black font-black font-black">{item.teacherName}</div></div>)}
                                </div>
                              ))}
                          </div>
                        );
      ...
      {proposals.length > 0 && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md flex items-center justify-center z-[60] p-2 md:p-4 font-sans overflow-hidden text-slate-900 font-black">
          <div className="bg-[#f8fafc] rounded-[1.5rem] md:rounded-[3.5rem] shadow-2xl border-2 md:border-4 border-blue-900 w-full max-w-6xl h-[95vh] flex flex-col overflow-hidden animate-in zoom-in duration-300">
            <div className="py-2 md:py-3 px-4 md:px-6 bg-white border-b-2 border-slate-200 flex items-center justify-between shrink-0 font-black font-black font-black">
              <div className="flex items-center gap-2 md:gap-3 font-black"><div className="w-8 h-8 md:w-10 md:h-10 bg-blue-50 text-blue-900 rounded-xl flex items-center justify-center shadow-inner font-black font-black font-black font-black"><GitBranch size={20} className="font-black font-black font-black"/></div><div><h3 className="text-sm md:text-xl font-black text-slate-800 font-black font-black font-black font-black">分析預覽</h3><p className="text-[10px] text-slate-400 font-bold">分析已完成</p></div></div>
              <button onClick={() => { setProposals([]); setPreviewProposal(null); }} className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-400 hover:bg-red-50 hover:text-red-500 transition-all font-black font-black"><X size={20}/></button>
            </div>
            <div className="flex-1 overflow-y-auto p-2 md:p-4 custom-scrollbar bg-slate-50 flex flex-col font-black font-black">
              {previewProposal ? (
                <div className="animate-in slide-in-from-right-4 duration-500 flex flex-col h-full font-black font-black font-black font-black font-black">
                    <div ref={exportRef} className="p-2 md:p-4 bg-slate-50 flex flex-col gap-2 md:gap-4">
                        <div className="bg-white p-3 md:p-4 rounded-xl md:rounded-2xl border-2 border-blue-600 shadow-lg shrink-0 font-black font-black font-black">
                            <div className="flex items-center gap-2 md:gap-3 mb-2 font-serif font-black font-black font-black font-black"><span className="text-blue-600 font-black text-[10px] md:text-xs uppercase bg-blue-50 px-2 py-1 rounded-full border border-blue-100 font-black font-black">方案：{previewProposal.title}</span><span className="text-orange-500 font-black text-lg md:text-2xl font-black">{previewProposal.letter} 方案</span></div>
                            <div className="flex flex-wrap gap-x-4 md:gap-x-6 gap-y-1 border-t border-slate-100 pt-2 font-serif font-black font-black font-black font-black">{previewProposal.actions.map((act, idx) => { const warning = getSlotWarning(act.t, act.d, act.p); return (<div key={idx} className="flex items-center gap-1 md:gap-2 text-slate-800 font-black text-[10px] md:text-base font-black"><span>{idx + 1}. [{act.t}] → {DAYS[act.d]} {PERIODS.find(p => p.id === act.p)?.label}</span>{warning && <span className="ml-1 px-1 py-0.5 border border-red-500 text-red-600 text-[8px] md:text-[10px] font-black rounded bg-red-50/50">{warning}</span>}</div>)})}</div>
                        </div>
                    ...
                    <div className="flex flex-wrap justify-center items-center gap-2 md:gap-3 py-3 md:py-4 mt-auto shrink-0 border-t border-slate-200 bg-white/50 rounded-b-[2.5rem] font-black font-black font-black font-black font-black font-black font-black font-black">
                        <button onClick={() => setPreviewProposal(null)} className="flex items-center gap-1 md:gap-2 px-3 md:px-5 py-2 md:py-3 bg-slate-200 text-slate-600 rounded-xl font-black text-xs md:text-lg hover:bg-slate-300 transition-all font-serif font-black font-black">返回列表</button>
                        <button onClick={handleSaveAsImage} className="flex items-center gap-1 md:gap-2 px-3 md:px-5 py-2 md:py-3 bg-white text-blue-600 border-2 border-blue-200 rounded-xl font-black text-xs md:text-lg hover:bg-blue-50 shadow-sm font-serif font-black font-black">另存圖片</button>
                        {isAdmin ? (<button onClick={executeFinalAdopt} className="flex items-center gap-2 px-6 md:px-10 py-2.5 md:py-3.5 bg-blue-900 text-white rounded-xl font-black text-sm md:text-xl shadow-lg hover:bg-blue-800 hover:-translate-y-1 transition-all border-b-4 border-blue-950 font-serif font-black">採用方案 <CheckCircle2 size={18}/></button>) : (<div className="flex flex-col items-center px-4 font-serif font-black font-black">{dbData.isApplyEnabled ? (<><span className="text-purple-700 font-black text-[10px] md:text-sm mb-1 uppercase animate-pulse font-black font-black font-black">如需正式調動：</span><Link to="/apply" className="bg-purple-600 text-white px-4 md:px-8 py-2 md:py-3 rounded-xl font-black text-xs md:text-lg hover:bg-purple-700 shadow-lg transition-all flex items-center gap-2 font-black font-black font-black font-black font-black">前往填報「自主調課表」 <ArrowRight size={16}/></Link></>) : (<div className="flex items-center gap-2 px-4 py-2 bg-slate-100 text-slate-400 rounded-xl font-black text-xs border-2 border-slate-200 font-black font-black font-black"><Lock size={14} className="font-black"/> 未開放填報</div>)}</div>)}
                        <button onClick={() => { setProposals([]); setPreviewProposal(null); }} className="flex items-center gap-1 md:gap-2 px-3 md:px-5 py-2 md:py-3 bg-red-50 text-red-500 border border-red-100 rounded-xl font-black text-xs md:text-lg hover:bg-red-500 hover:text-white transition-all font-serif font-black font-black font-black"><Trash2 size={16} className="font-black"/> 放棄</button>
                    </div>
                </div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6 font-black font-black font-black font-black font-black font-black text-slate-900">
                  {proposals.map((p, i) => (
                    <div key={i} className={`p-4 md:p-6 rounded-2xl md:rounded-3xl border-2 bg-white flex flex-col justify-between transition-all shadow-lg group hover:border-blue-900 hover:shadow-2xl font-black font-black font-black font-black`}>
                      <div className="flex items-start gap-3 md:gap-4 mb-3 md:mb-4 font-black font-black font-black font-black"><div className="mt-1 font-black font-black font-black font-black font-black">{p.type === 'MOVE' ? <CheckCircle2 className="text-green-600 font-black font-black" size={24} /> : p.type === 'SWAP' ? <ArrowRightLeft className="text-indigo-600 font-black font-black" size={24} /> : p.type === 'TRIANGLE' ? <RefreshCw className="text-purple-600 animate-spin-slow font-black font-black font-black" size={24} /> : <XCircle className="text-red-600 font-black font-black font-black" size={24} />}</div>
                        <div className="flex-1 font-black font-black font-black font-black"><div className="flex justify-between items-center mb-1 md:mb-2 font-serif font-black font-black font-black font-black font-black"><span className="font-black text-sm md:text-xl text-slate-800 font-black font-black font-black">{String.fromCharCode(65 + i)} 方案：{p.title}</span><span className={`text-[8px] md:text-[10px] font-black px-1.5 md:px-2 py-0.5 rounded-full text-white font-sans ${p.color==='blue'?'bg-blue-600':p.color==='indigo'?'bg-indigo-600':p.color==='purple'?'bg-purple-600':'bg-red-600'}`}>{p.impact}</span></div>
                          <div className="space-y-1 mt-1 md:mt-2 font-serif font-black font-black font-black font-black">{p.actions?.map((act, idx) => { const warning = getSlotWarning(act.t, act.d, act.p); return (<div key={idx} className="flex items-center gap-1 md:gap-2 flex-wrap text-slate-600 font-bold text-xs md:text-lg font-black font-black"><span>{idx + 1}. [{act.t}] → {DAYS[act.d]} {PERIODS.find(per => per.id === act.p)?.label}</span>{warning && <span className="px-1.5 py-0.5 border border-red-500 text-red-600 text-[8px] md:text-[10px] font-black rounded bg-red-50/50 whitespace-nowrap font-sans font-black font-black font-black font-black">{warning}</span>}</div>);})}</div>
                        </div>
                      </div>
                      {!p.disabled && <button onClick={() => setPreviewProposal({ ...p, letter: String.fromCharCode(65 + i) })} className="mt-2 md:mt-3 w-full bg-slate-800 text-white py-2.5 md:py-4 rounded-xl font-black text-sm md:text-xl hover:bg-black transition-all flex items-center justify-center gap-2 font-serif font-black font-black text-white font-black font-black font-black font-black font-black">預覽方案結果 <ArrowRight size={18} className="font-black"/></button>}
                    </div>
                  ))}
                </div>
              )}
